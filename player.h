...
public:
    explicit Player(const Cube *head, const Cube *body, const Cube *leftArm, const Cube *rightArm, const Cube *leftLeg, const Cube *rightLeg)
    ...
    explicit Player(const Texture *tex)
        : startTime(static_cast<float>(glfwGetTime())), state(State::STAND), v(1.0) {
...
public:
    explicit PlayerController(Player *player, const std::pair<int, int> &pos,
    ...
    void reset(const std::pair<int, int> &pos, const std::pair<int, int> &dir = {1, 0});
};

...
namespace greedy {
    ...
    void GreedyRunner::buildTree(int x, int y) {
    ...
    void GreedyRunner::genPath(int x, int y, int u, int v, std::deque<std::pair<int, int>> &ans) {
        if(x == u && y == v){
...
#undef ratio
    ...
    std::vector<std::pair<int, int> > GreedyRunner::getPath() const {
        assert(finish);
        return path;
    }

    int GreedyRunner::getW() const {
        assert(finish);
        return w;
    }
} // greedy
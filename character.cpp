...
void Character::draw(const glm::mat4 &model, const glm::mat4 &view, const glm::mat4 &projection) const {
...
void Character::setState(const State s) {
    if(s == state) return;
    state = s;
    startTime = static_cast<float>(glfwGetTime());
}

void Character::setSpeed(const float v) {
    this->v = v;
}
...
void CharacterController::setSpeed(const float v) const {
    player->setSpeed(v);
}

void CharacterController::addAction(const std::tuple<int, int, int> &op) {
    actionQueue.emplace_back(op);
}

void CharacterController::changeAction(const std::tuple<int, int, int> &op) {
...
void CharacterController::update(const glm::mat4 &view, const glm::mat4 &projection) {
    ...
    auto draw = [&]() {
        ...
        if(op == -1){
            player->setState(CharacterState::STAND);
        ...
        if(op == 2){
            if((t - lastStartTime) * KV * player->getSpeed() >= std::numbers::pi){
                ...
                player->setState(CharacterState::FIGHT);
            }
            player->draw(genModelMat(pos, direction), view, projection);
            return;
        }
...
bool CharacterController::isAvailable() const {
    const auto [op, u, v] = currentOption;
    ...
    //攻击
    if(op == 2){
        return (t - lastStartTime) * KV * player->getSpeed() >= std::numbers::pi && player->getState() == CharacterState::STAND;
    }
    // 转向
    if(op == 3){
        return true;
...
std::pair<float, float> CharacterController::getCurrentPos() const {
    return currentPos;
}

bool CharacterController::isQueueEmpty() const {
